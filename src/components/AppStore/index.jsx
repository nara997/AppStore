...
const [searchInput, setSearchInput] = useState("")
   ...
   const finalAppsList = appsList.filter(eachitem1 =>
      eachitem1.category === activeTabId
    )

    const filteredSearchList = finalAppsList.filter(eachitem2 =>
        eachitem2.appName.toLowerCase().includes(searchInput.toLowerCase())
        )
    ...
    return (
        <div className="app-container">
            <div className="app-store">
                ...
                <ul className="tabs-conatiner">
                    {
                      tabsList.map(eachTabItem =>
                        <TabItem key={eachTabItem.tabId} tabDeatils={eachTabItem} isActive={activeTabId === eachTabItem.tabId} setActiveTabId={setActiveTabId}/>
                        )
                    }

                </ul>
                <ul className="Apps-conatiner">
                    {
                      filteredSearchList.map(eachAppItem =>
                        <AppItem key={eachAppItem.appId} eachAppItem ={eachAppItem}/>
                        )
                    } 

                </ul>
                {filteredSearchList.length === 0 && <h1 className="no-apps-heading">No Apps To Show</h1>}
            </div>
